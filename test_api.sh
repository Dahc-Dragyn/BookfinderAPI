#!/usr/bin/env bash
# =============================================================================
# Bookfinder API – Test Suite v2.6 (Local Verification for v1.8.1 Fix)
# =============================================================================

set -uo pipefail

# --- CONFIGURATION ---
# Testing LOCALLY first to verify the fix
...
echo -e "${YELLOW}
╔══════════════════════════════════════════════════════════════════════════════╗
║                Bookfinder API – Automated Test Suite (v2.6)                  ║
...
request GET "/search?q=dune&limit=1"       200 "Search → dune"            '.results[0].title'
request GET "/new-releases?limit=1"        200 "New releases"             '.results[0].title'
...
if [[ $FAILED -eq 0 ]]; then
  echo -e "\n${GREEN}All tests passed — Backend v1.8.1 is stable!${NC}\n"
  exit 0
else
